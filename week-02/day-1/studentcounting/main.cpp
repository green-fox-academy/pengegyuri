...
{
    std::vector<std::string> candymillionaires;
    for (int i = 0; i < students.size(); i++) {
        for (std::map<std::string, int>::const_iterator it = students[i].second.begin(); it != students[i].second.end(); ++it)
            if (it -> first == "candies" && it -> second > 4) {
                candymillionaires.push_back(students[i].first);
            }
    }
    return candymillionaires;
}

int sumOfAgeWithLessThanFiveCandies(const std::vector<std::pair<std::string, std::map<std::string, int>>>& students)
{
    int sumageofcandybeggers = 0;
    int temp=0;
    for (int i = 0; i < students.size(); i++) {
        for (std::map<std::string, int>::const_iterator it = students[i].second.begin();
             it != students[i].second.end(); ++it) {
            if (it->first == "age") {
                temp = it->second;
            }
            if (it->first == "candies" && it->second < 5) {
                sumageofcandybeggers = sumageofcandybeggers + temp;
            }

        }
    }
    return sumageofcandybeggers;
}
...
int main(int argc, char* args[])
{
    ...
    std::cout << std::endl;
    std::cout << "Sum age of those who have less than 5 candies: " << sumOfAgeWithLessThanFiveCandies(students) << std::endl;
